...
class APIListTests(unittest.TestCase):
    def test_saved_metadata_order_epoch_ids(self):
        with tempfile.TemporaryDirectory() as directory:
            records = [
                {'timestamp': datetime(1970, 1, 1, 2, 0, 1, 234567, tzinfo=timezone(timedelta(hours=2))),
                 'name': 'Lamp'},
                {'timestamp': datetime(1970, 1, 1, tzinfo=timezone.utc), 'name': 'First spectrum'},
                {'timestamp': datetime(1970, 1, 2, tzinfo=timezone.utc),
                 'name': 'No calibration'},
            ]
            for i, record in enumerate(records):
                record = spectrum_record(**record)
            ...
            self.assertEqual(entries[2]['name'], 'First spectrum')
