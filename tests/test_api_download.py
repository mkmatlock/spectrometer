...
class DownloadTests(unittest.TestCase):
    ...
    def test_large_legacy_download_is_complete_in_bounded_writes(self):
        raw = np.arange(1536 * 2048, dtype=np.uint8).reshape(1536, 2048)
        timestamp = datetime(2026, 9, 15, 12, 34, 56, tzinfo=timezone.utc)
        handler = self.download_handler({
            'timestamp': timestamp, 'name': 'Legacy spectrum',
            'raw_camera_output': raw, 'spectrum_bar': b'\x00\xff',
        ...
        self.assertEqual(downloaded['raw_camera_output']['dtype'], 'uint8')
        self.assertEqual(downloaded['raw_camera_output']['shape'], [1536, 2048])
        self.assertEqual(base64.b64decode(downloaded['raw_camera_output']['data']), raw.tobytes())
    ...
    def test_interrupted_download_never_appends_an_error_response(self):
        record = {'raw_camera_output': np.zeros((256, 1024), np.uint8)}
    ...
    def test_serialization_failure_sends_a_single_complete_error(self):
        handler = self.download_handler({'invalid_value': object()})
